...
import Modal from '../../../components/compliance/record/modal'

import { useStore } from 'effector-react'

import Form, { TextArea, Select, toOpt, Input} from '../../../components/form'

import { useQuery } from '../../../components/compliance/util'
...
export default function({loadP, setShowCases, ...props}) {
  let { project_id, section_id, attr_id, case_id} = useParams()

  const submitted = (project_id, section_id, attr_id=0, id, data, loadP) => {
    const cb = (resp) => {
  ...
  return (
    <Modal close={() => setShowCases(false)}>
