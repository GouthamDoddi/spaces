...
const { load, update, create } = makeStore(({ project_id }) =>
  project_id ? `rev-projects/${project_id}` : 'rev-projects'
);
const { load: loadEntities } = makeStore('entities');

const MasterProjectProfile = () => {
  ...
  const [data, setData] = useState({
    ...
    short_name: '',
    project_type_id: projectCategoryTypes[1].value,
    owner_id: '',
    sponsor_id: 1,
    logo: null,
  });
    ...
    if (!hasErrors) {
      if (project_id !== 'new') {
        update({
          data,
          cb: () =>
            history.push(`/projects/${project_id}/compliance-project-details`),
          project_id,
        });
      } else {
        create({
          data,
          cb: (data) =>
            history.push(`/projects/${data.id}/compliance-project-details`),
        });
      }
    }
  };
          ...
          <div className="flex_col_sm_6">
            ...
            <div className="flex_row">
              <div className="flex_col_sm_4">
                <div className="form_field_wrapper">
                  <label className="form_label">Sponsor</label>
                  <div className="text_field_wrapper">
                    <select
                      name="sponsor_id"
                      value={sponsor_id}
                      onChange={handleChange}
                    >
                      {Object.values(projectSponsorOptions).map(
                        ({ label, value }) => (
                ...
                <div className="form_field_wrapper">
                  ...
                  <div className="text_field_wrapper">
                    <select
                      ...
                      onChange={handleChange}
                    >
                      {Object.values(projectCategoryTypes).map(
                        ({ value, label }) => (
                          <option id={value} value={value}>
                            {label}
                          </option>
            ...
            <div className="form_field_wrapper">
              ...
              <DropZone
                ...
                sub_label={
                  'Allowed file formats are PNG, JPG within 10MB, Suggested dimensions are W-300px & H-300px'
                }
              />
          ...
          <div className="flex_col_sm_6">
            <div className="form_field_wrapper ar">
              ...
              >
                ...
                <div className="text-right">
                  <span className="limit">
                    بقي {100 - project_name_ar?.length} حرف
                  </span>
                </div>
              </div>
