...
const { load } = makeStore(({ compliance_project_id }) => `rev-compl-projects/${compliance_project_id}/sections-report`);
const { load: loadCases } = makeStore(({ project_id }) => `rev-projects/${project_id}/cases`);

const Sections = ({ onSubmit, selected, setTableProps}) => {
  const { project_id } = useParams();
  const [sections, setSections] = useState([]);

  useEffect(() => {
    if (selected['1']?.id) {
      load({ compliance_project_id: selected['1'].id }, (data) => setSections(data));
    }
      ...
      <div className="flex_row">
        <div className="flex_col_sm_12">
          <Table>
            ...
            <Tbody>
              {sections.map(({ section_id, section_name, attribute_count, parameter_count, completed, not_tested }) => (
                <tr key={section_id} onClick={() => onSubmit({ name: section_name, id: section_id })}>
                  <td>{section_name}</td>
