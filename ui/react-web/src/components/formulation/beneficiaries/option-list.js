...
export default function({title, description, onClicked, dataStore, sid, path, policyStore, bidsKey}) {
  ...
  const changed = (val, id) => {
    ...
    if (val) {
      put(`formulation/${policy_id}/add_${path}/${id}`, {success})
    } else {
      put(`formulation/${policy_id}/remove_${path}/${id}`, {success})
    }
  }
