module App
  class<<self
    ...
    def load!
      ...
      audit_db_url = "postgres://npdnvsjj:KH-TxJ577bSuomJ0IY8S0QeupHhrnRer@balarama.db.elephantsql.com:5432/npdnvsjj"
      @audit_db = Sequel.connect(db_url, logger: Logger.new(STDOUT), max_connections: NUMBER_OF_CONNECTIONS, after_connect: Proc.new{puts "************ Connected ************ "})
      @db = Sequel.connect(db_url, logger: Logger.new(STDOUT), max_connections: NUMBER_OF_CONNECTIONS, after_connect: Proc.new{puts "************ Connected ************ "})
      require_blob('lib/**/*.rb')
      setup_sequel!
